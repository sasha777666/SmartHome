...
// ОпределитьРазмерФайлаКаталога, Возвращает размер в байтах  
// Параметры:
// ПолныйПуть - Строка - Полный путь к файлу или каталогу источнику (без последнего слеша, пример C:\Папка1)	 
Функция ОпределитьРазмерФайлаКаталога(ПолныйПуть) Экспорт
	
	Попытка 
		Файл = Новый Файл(ПолныйПуть);
		ИтоговыйРазмерВБайтах = 0;
		Если Файл.ЭтоКаталог() Тогда 
			НайденныеФайлы = НайтиФайлы(ПолныйПуть, "*.*", Истина);	
			Для Каждого Файл Из НайденныеФайлы Цикл
				ИтоговыйРазмерВБайтах = ИтоговыйРазмерВБайтах + ?(Файл.ЭтоКаталог(), 0, Файл.Размер()); 	
			КонецЦикла;
		Иначе
			ИтоговыйРазмерВБайтах = Файл.Размер();
		КонецЕсли;
	Исключение 
		ИтоговыйРазмерВБайтах = Неопределено;
		ОписаниеОшибки = "Сбой при определении размера файла/каталога, Путь = " + ПолныйПуть + ". Описание ошибки: " + ОписаниеОшибки();
		#Если Клиент Тогда
			Сообщить(ОписаниеОшибки);
		#ИначеЕсли Сервер Тогда
			ЗаписьВЖурналОшибок("ОбщегоНазначения", "ОпределитьРазмерФайлаКаталога", ОписаниеОшибки);	
		#КонецЕсли	
	КонецПопытки;
	
	Возврат ИтоговыйРазмерВБайтах;
