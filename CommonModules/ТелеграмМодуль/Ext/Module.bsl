...
Процедура ПолучитьНовыеСообщения(Оборудование = Неопределено)Экспорт 
	...
	Если ЗначениеЗаполнено(Справочники.ТелеграмПараметры.Последний_update_id.Значение)Тогда
		OffSet = "?offset="+Формат(Справочники.ТелеграмПараметры.Последний_update_id.Значение + 1, "ЧГ=");
	Иначе
		OffSet = "";
	КонецЕсли;
	
	Ключ = Справочники.ТелеграмПараметры.КлючAPI.Значение;
	Сервер = "api.telegram.org";
	
	Если Справочники.ТелеграмПараметры.ПроксиИспользовать.Значение = Истина Тогда 
	...
	Если ОтветСоотв["ok"] Тогда
		...
		Если ЗначениеЗаполнено(update_idПоследнегоСообщения) Тогда 
			СправочникОбъектupdate_id = Справочники.ТелеграмПараметры.Последний_update_id.ПолучитьОбъект();
			СправочникОбъектupdate_id.Значение = Число(update_idПоследнегоСообщения);
			СправочникОбъектupdate_id.Записать();
		КонецЕсли;
	КонецЕсли;
...
Функция ОбработкаСообщенияПоШаблонамТелеграмПараметров(СсылкаДокВхСообщение)
	...
	Если ЗначениеЗаполнено(Справочники.ТелеграмПараметры.ШаблонСоздатьИОткрытьГруппуСообщений.Значение)
	И НРег(ТекстСообщения) = НРег(Справочники.ТелеграмПараметры.ШаблонСоздатьИОткрытьГруппуСообщений.Значение) Тогда 
	...
	ИначеЕсли ЗначениеЗаполнено(Справочники.ТелеграмПараметры.ШаблонЗакрытьТекущуюГруппуСообщений.Значение)
	И НРег(ТекстСообщения) = НРег(Справочники.ТелеграмПараметры.ШаблонЗакрытьТекущуюГруппуСообщений.Значение) Тогда 
	...
	ИначеЕсли ЗначениеЗаполнено(Справочники.ТелеграмПараметры.ШаблонДеактивироватьТекущийИИ.Значение)
	И НРег(ТекстСообщения) = НРег(Справочники.ТелеграмПараметры.ШаблонДеактивироватьТекущийИИ.Значение) Тогда 
...
Функция СоздатьИОтправитьДокСообщениеИсходящие(СтруктураСообщения) Экспорт
	...
	Если СтруктураСообщения.Свойство("Текст") Тогда 
		нДок.Текст = СтруктураСообщения.Текст;
	КонецЕсли;
	//Текст--		
	...
	Попытка 
		...
		//Вложения++
			...
			//Фото++
				...
				КонецЕсли;
			//Фото--
			ИначеЕсли Вложение.Тип = "document" Тогда //Отправляем как фотографию
			//Файл++
