...
Процедура СохранитьСнимок()
	...
	Если ТекДанныеКомандыТранслировать.Дата + 3 >= ТекущаяДата() И ТекДанныеКомандыТранслировать.Данные = 1 Тогда // включена трансляция и фото уже получено
		Ответ2 = РегистрыСведений.ТекущиеДанныеОборудования.Получить(Новый Структура("Оборудование, ОбъектДанных",Оборудование, "Трансляция")).Хранилище.Получить(); // получим фото из регистра	
	Иначе
		Ответ2 = ПолучитьФотоВнутренняя(); // получим фото с камеры	
	КонецЕсли;
