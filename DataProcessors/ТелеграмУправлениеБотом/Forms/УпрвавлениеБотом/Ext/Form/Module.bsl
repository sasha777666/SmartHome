...
Процедура ОбновитьСообщения()
	...
	СтарыйТекстСообщений =  ЭтотОбъект.СообщенияТекст;
 
	ПроверитьОбновитьСообщенияНаСервере(пКонтакт);	
	
	Если НЕ СтарыйТекстСообщений = ЭтотОбъект.СообщенияТекст Тогда // текст сообщений изменился
		ПоследнийСимвол = СтрДлина(ЭтотОбъект.СообщенияТекст);
		Элементы.СообщенияТекст.УстановитьГраницыВыделения(ПоследнийСимвол, ПоследнийСимвол);	
	КонецЕсли;
...
Процедура ОтправитьСообщение(Команда)
	...
	Для Каждого нВыдСтроки Из Элементы.СписокКонтактов.ВыделенныеСтроки Цикл
		пКонтакт = Неопределено;
		пКонтакт = СписокКонтактов.Получить(нВыдСтроки);	
